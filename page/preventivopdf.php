...
/* Recupero l'id dell'ultimo preventivo inserito */
$id_preventivo = (int)$_GET['id_preventivo'];
//var_dump($id_preventivo);
//exit;

/* Recupero i dati del preventivo */
...
if ($count == 0) {
echo "<p style='text-align:center;margin-top: 10%;'>Non esistono preventivi corrispondenti all'ID: $id_preventivo</p>";

}else{

while ($row = $rs1->fetch_array(MYSQLI_ASSOC)) {
if(isset($row['id_preventivo'])){$id_preventivo = mysqli_real_escape_string($mysqli, $row['id_preventivo']);}else{$id_preventivo ="";}
if(isset($row['cliente'])){$cliente = mysqli_real_escape_string($mysqli, $row['cliente']);}else{$cliente ="";}
if(isset($row['email'])){$email = mysqli_real_escape_string($mysqli, $row['email']);}else{$email ="";}
...
if(isset($row['n_accettatore'])){$n_accettatore = mysqli_real_escape_string($mysqli, $row['n_accettatore']);}else{$n_accettatore ="";}
...
<form name="preventivo_pre_stampa" class="form" method="POST" target="_blank" action="<?php echo "pdf_preventivo.php?id_preventivo=$id_preventivo"; ?>">
...
<div class="card-commesse">
  ...
  <div class="card-block">
	...
	<div class="row">
		...
		<div class="col-md-4">
			  <div class="input-group mb-2 mr-sm-2 mb-sm-0">
				...
				<input type="text" name="id" disabled="disabled" id="id" value="<?= $id_preventivo ?>" size="10" readonly="readonly">
			  </div>
		</div>
	...
	<div class="row">
		...
		<div class="col-md-4">
			  <div class="input-group">
				<span class="input-group-addon"><i class="fa fa-email" aria-hidden="true"></i></span>
				<input type="text" class="form-control" name="email" required="required" placeholder="Email" value="<?php echo $email ?>" readonly="readonly">
			  </div>
		</div>
...
<div class="card-commesse">
  ...
  <div class="card-block">
	...
	<div class="row">
		...
		<div class="col-md-4">
		 <label class="sr-only" for="n_accettatore">Preventivata da:</label>
			  <div class="input-group mb-2 mr-sm-2 mb-sm-0">
				<span class="input-group-addon"><i class="fa fa-user" aria-hidden="true"></i></span>
				<input type="text" class="form-control" name="n_accettatore required="required" placeholder= "Preventivata da:" id="n_accettatore" value="<?php echo $n_accettatore ?>" readonly="readonly">
			  </div>
		</div>			
	</div>
		...
		<div class="col-md-4">
		 <label class="sr-only" for="km">Km</label>
			  <div class="input-group mb-2 mr-sm-2 mb-sm-0">
				...
				<input type="text" class="form-control" name="km" required="required" placeholder= "Km" id="km" value="<?php echo $km ?>" readonly="readonly">
			  </div>
		</div>	
	</div>
...
<div class="card-commesse">
  ...
  <div class="card-block">
	...
	<div class="text-right">
		<button type="submit" name="submit" class="btn btn-success right">Stampa Pdf Preventivo</button> 
	</div>
