...
/* Recupero i dati del preventivo inserito */
...
if(isset($_POST['n_accettatore'])){$n_accettatore = mysqli_real_escape_string($mysqli, $_POST['n_accettatore']);}else{$n_accettatore ="";}
...
$email = trim(strip_tags(strtoupper($email)));
$tel = trim(strip_tags(strtoupper($tel)));
$totale = trim(strip_tags(strtoupper($totale)));
$n_accettatore = trim(strip_tags(strtoupper($n_accettatore)));
...
$messaggio = "<div class='success'>Preventivo aggiornato con successo! Attendi..</div>";
echo "<meta http-equiv='refresh' content='3;url=index.php?page=preventivopdf&id=$id_preventivo'>";
...
<div class="row">
		...
		<div class="col-md-4">
				 <label class="sr-only" for="n_accettatore">Preventivo eseguito da:</label>
				 <div class="input-group mb-2 mr-sm-2 mb-sm-0">
					<span class="input-group-addon"><i class="fa fa-users" aria-hidden="true"></i></span>
						...
						<?php 
							...
							$sq5 = "SELECT * FROM accettatore WHERE n_accettatore != '$n_accettatore' ORDER BY n_accettatore ASC";
...
<fieldset>
<legend>Interventi Preventivati (Possono subire variazioni)</legend>
<table id="commessa">
<tbody>
...
<!-- Fine: Codice di Domenico -->
	...
	<div class="text-right">
		<button type="submit" name="submit" class="btn btn-success right">Aggiorna Preventivo</button> 
	</div>
