...
$indirizzo = mysqli_real_escape_string($mysqli, strtoupper($row['indirizzo']));
$veicolo = mysqli_real_escape_string($mysqli, strtoupper($row['veicolo']));
$piva = mysqli_real_escape_string($mysqli, strtoupper($row['piva']));
$tel = mysqli_real_escape_string($mysqli, strtoupper($row['tel']));
$targa = mysqli_real_escape_string($mysqli, strtoupper($row['targa']));
$telaio = mysqli_real_escape_string($mysqli, strtoupper($row['telaio']));
...
$pagamento = mysqli_real_escape_string($mysqli, strtoupper($row['pagamento']));
...
function Header()
{
	...
	$this->Cell(282,5,'Indirizzo, civico',0,1,'C');
	$this->Ln(3);
	$this->Cell(302,5,'CAP - Citta (PR)',0,1,'C');
	$this->Ln(3);
	$this->Cell(279,5,'Tel/Fax 021230000',0,1,'C');
	$this->Ln(3);
	$this->Cell(298,5,'http://www.indirizzoazienda.com',0,0,'C');
    // Line break
    $this->Ln(20);
}
...
function Footer()
{
    ...
    // Page number
    $this->Cell(0,10,'Vi ringraziamo per la preferenza accordata',0,0,'C');
}
}
*/
...
$p->Cell(185, 10, 'Anagrafica Cliente' ,1,1,'C');
$p->Ln(3);
...
$p->Ln(3);
$p->Cell(100, 5, ' Km : '.$km.'',0);
$p->Cell(80, 5, 'Telaio : '.$telaio.'',0,1);
$p->Ln(3);
$p->Cell(100, 5, ' Cliente : '.$cliente.'',0);
$p->Cell(80, 5, 'Indirizzo : '.$indirizzo.'',0,1);
$p->Ln(3);
$p->Cell(100, 5, ' P.Iva : '.$piva.'',0);
...
$p->Ln(43);
$p->Cell(145, 5, 'Data '.$data.'',0);
$p->Cell(0, 5, ' Totale = € '.$totale.'',0,1);
...
$p->Cell(0,10,'LA RINGRAZIAMO PER LA PREFERENZA ACCORDATA',0,1,'C');
$p->Cell(0,0,'SE E\'INTERESSATO AL NOLEGGIO AUTO - CHIEDA INFO IN ACCETTAZIONE',0,0,'C');
